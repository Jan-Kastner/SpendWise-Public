...
namespace SpendWise.Common.Tests.Helpers
{
    ...
    /// <see cref="IUnitOfWork"/>, and BLL services. This class handles the setup 
    /// and teardown of the test environment by managing the database context and related services.
    /// </summary>
    public class ServicesTestsBase : IAsyncLifetime
    {
        ...
        {
            await _SpendWiseDbContextSUT.Database.EnsureDeletedAsync().ConfigureAwait(false);
            await _SpendWiseDbContextSUT.Database.EnsureCreatedAsync().ConfigureAwait(false);
        }
        ...
        {
            await _SpendWiseDbContextSUT.Database.EnsureDeletedAsync().ConfigureAwait(false);
            await _SpendWiseDbContextSUT.DisposeAsync().ConfigureAwait(false);
        }
    }
}